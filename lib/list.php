...
if(json_decode($content,1)["error_code"]==31045){
	...
	echo '<div class="col-md-8 col-md-offset-2 " role="main"><ol class="breadcrumb"><li><a href="./?m=list&order='.$order.'&by='.$by.'&page='.$list.'&path=/">根目录</a></li>'.showlistname(urldecode($path)).'</ol><center><div class="table-responsive"><table  class="table table-hover table-striped table-condensed table-responsive" ><th class="mdl-data-table__cell--non-numeric"  style="width:7%">类型</th><th class="mdl-data-table__cell--non-numeric ">文件名称</th><th class="mdl-data-table__cell--non-numeric"  style="width:5%">下载1</th><th class="mdl-data-table__cell--non-numeric"  style="width:5%">下载2</th>';
	$x=0;
	foreach($b["list"] as $c){
		if($c["isdir"]==1){
			echo '<tr><td class="mdl-data-table__cell--non-numeric">文件夹</td><td class="mdl-data-table__cell--non-numeric"><a href="./?m=list&by='.$by.'&order='.$order.'&path='.urlencode($c["path"]).'&page=1">'.$c["server_filename"].'</a></td><td class="mdl-data-table__cell--non-numeric"></td><td class="mdl-data-table__cell--non-numeric"></td></tr>';
		}
		else{
			echo '<tr><td class="mdl-data-table__cell--non-numeric">文件</td><td class="mdl-data-table__cell--non-numeric">'.$b["list"][$x]["server_filename"].'</td><td class="mdl-data-table__cell--non-numeric"><a href="./?m=car&path='.urlencode($b["list"][$x]["path"]).'"target="_blank">链接1</a></td><td class="mdl-data-table__cell--non-numeric"><a href="./?m=suv&path='.urlencode($b["list"][$x]["path"]).'"target="_blank">链接2</a></td></tr>';
		}
		$x++;
	}
}
